---
title: "Forecasting Aemo STDS Assignment 3"
...
# Auto Regressive Integration of Moving Averages 
...
## TSFeatures { .tabset .tabset-pills .tabset-fade}
...
## Plotting ARIMA forecasts {.tabset .tabset-pills .tabset-fade}
...
## Box-Cox Transformation {.tabset .tabset-pills .tabset-fade}
...
accuracy(weekly_arima_diff)
```

## ACF {.tabset .tabset-pills .tabset-fade}
...
DT::datatable(auto_arima_accuracy)
```
...
```{r echo=FALSE}
DT::datatable(snaive_accuracy_10_weeks)
```
...
```{r echo=FALSE}
DT::datatable(snaive_accuracy_8_weeks)
```
...
```{r echo=FALSE}
DT::datatable(tbats_accuracy_4_weeks)
```
...
```{r echo=FALSE}
DT::datatable(tbats_accuracy_2_weeks)
```
...
DT::datatable(accuracy(aemo_weekly_auto_arima))
```

# Conclusions

Initially it was tempting to explore the ARIMA model because it offers the tantalizing prospect of being uninfluenced by potentially erroneous variables, and being able to predict future values based off of past values. A timeseries statistician using an ARIMA model needs to be prepared to abandon the notion that there are explanatory variables other than time that will predict the value of a dependent variable. In the case of AEMO energy demand data, I believe that the answer lies somewhere in the grey area; that while we can achieve a reasonable degree of accuracy (or a reasonable low Mean Absolute Standard Error), I intend to continue exploring these methods and beyond. My next exploration of statistical methods will involve looking a possible ways to use an ARIMAX model; essentially an ARIMA model that also factors in multiple explanatory/independent variables alongside the timeseries forecasting of the ARIMA model. Because of the intuitive and rational sense behind the idea that energy demand is likely a function of time as well as various other explanatory variables (as we and also Hyndman have found, temperature data can play a significant role in energy demand) I believe that an intelligent ARIMA model that factors in the linear relationships of independent and dependent variables will achieve a greater level of prediction accuracy than the ARIMA models explored in this submission. 
